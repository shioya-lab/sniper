...
void hashed_perceptron::last_branch_result(champsim::address pc, champsim::address branch_target, bool taken, uint8_t branch_type)
{
    ...
    if (m_debug_pc == pc.to<uint64_t>()) {
      fprintf(m_debug_fp, "HP: IP=0x%lx, Target=0x%lx, Predicted=%d, Actual=%d, Result=%d\n", pc.to<uint64_t>(), branch_target.to<uint64_t>(), (last_result.yout >= THRESHOLD), taken, prediction_correct);
    }
    ...
    if (!prediction_correct) {
        ++m_incorrect_per_ip[pc.to<uint64_t>()]; // count mispredictions per IP
    }
