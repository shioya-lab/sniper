...
#define PREF_DEBUG_PRINTF(...) { if (m_enable_log && (m_pref_target_log == 0 || pc == m_pref_target_log)) { fprintf(stderr, __VA_ARGS__); }}
...
VecPrefetcher::getNextAddress(IntPtr current_address, Core::mem_op_t mem_op_type, IntPtr pc, uint64_t uop_idx, core_id_t _core)
{
  ...
  PREF_DEBUG_PRINTF("  %s VecPrefetcher::getNextAddress(pc=%08lx, addr=%08lx) start :\n", configName.c_str(), pc, current_address);
  ...
  for (size_t i = 0; i < m_vec_stride_table.size(); i++){
    ...
    if (entry->last_uop_idx + 1 == uop_idx) {
      if (entry->last_addr == current_block) {
        PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Vector. pc=%08lx same address access. entry[%ld].{last_addr = %08lx, base_addr=%08lx} (degree = %d, vec_size = %d)\n",
        ...
        entry->last_addr = current_block;
          PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Vector. pc=%08lx same pc sequential update. entry[%ld].{last addr = %08lx, base_addr=%08lx} (degree = %d, vec_size = %d)\n",
                  configName.c_str(), pc, i, entry->last_addr, entry->base_addr, entry->vec_degree(), entry->vec_size);
      }
    ...
    PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Vector. pc = %08lx, base_addr = %08lx, last_addr = %08lx, stride = %08lx, entry index = %ld, confidence = %d\n",
              configName.c_str(),
              pc, entry->base_addr, entry->last_addr, entry->stride, i, entry->confidence
              );

    if (entry->base_addr + entry->stride == current_address) {
      PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Vector. pc = %08lx, hits entry_index=%ld, base_addr=%08lx, stride=%08lx, vec_size=%d\n",
                configName.c_str(), pc, i, entry->base_addr, entry->stride,  entry->vec_size);
      if (entry->can_prefetch()) {
        PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Vector. pc = %08lx, canprefetch = 1\n", configName.c_str(), pc);
        for (unsigned d = 0; d < entry->m_degree; d++) {
          ...
          for (unsigned vd = 0; vd < entry->vec_degree(); vd++) {
            ...
            if (std::find(addresses.begin(), addresses.end(), prefetch_addr) == addresses.end()) {
              ...
              PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Vector. pc = %08lx, push_address entry index = %ld, prefetch addr = %08lx (degree = %d, vec_size = %d)\n",
                          configName.c_str(), pc, i, prefetch_addr, entry->vec_degree(), entry->vec_size);
            } else {
              PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Vector. pc = %08lx, prefetch address already existed. filtered, prefetch addr = %08lx\n",
...
std::vector<IntPtr> VecPrefetcher::getScalarNextAddress(IntPtr pc, IntPtr current_address)
{
  ...
  for (size_t i = 0; i < m_scalar_stride_table.size(); i++){
    ...
    PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Scalar current_address pc = %08lx, base_addr = %08lx, last_addr = %08lx, stride = %08lx, entry index = %ld, confidence = %d\n",
            configName.c_str(),
            pc, entry->base_addr, entry->last_addr, entry->stride, i, entry->confidence
            );
    ...
    if (entry->stride != 0 && entry->base_addr + entry->stride == current_address) {
      ...
      if (entry->can_prefetch()) {
        ...
        for (unsigned d = 0; d < entry->m_degree; d++) {
          ...
          PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Scalar push_address entry index = %ld, prefetch addr = %08lx\n",
...
void VecPrefetcher::AllocateVecStride (const IntPtr pc, const IntPtr current_address)
{
  ...
  if (m_vec_stride_table.size() >= m_vec_stride_table_size) {
    ...
    PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Vector remove entry. pc = %08lx, addr = %08lx (degree = %d, vec_size = %d)\n",
  ...
  PREF_DEBUG_PRINTF(" %s: VecPrefetcher::AllocateVecStride::pc = %08lx, current_address = %08lx\n",
          configName.c_str(), pc, current_address);
}
...
void VecPrefetcher::AllocateScalarStride (const IntPtr pc, const IntPtr current_address)
{
  ...
  if (m_scalar_stride_table.size() >= m_scalar_stride_table_size) {
    ...
    PREF_DEBUG_PRINTF("   %s: VecPrefetcher::Scalar remove entry. pc = %08lx, addr = %08lx\n",
  ...
  PREF_DEBUG_PRINTF(" %s: VecPrefetcher::AllocateScalarStride::pc = %08lx, current_address = %08lx\n",
          configName.c_str(), pc, current_address);
}
