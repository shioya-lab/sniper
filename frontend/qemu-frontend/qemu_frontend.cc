...
class FrontendOptions<QemuFrontend> : public OptionsBase<QemuFrontend>
{
   ...
   std::string cmd_summary()
   {
      return "Options:\n"
             ...
             "  flow_control=<num> - instructions before sync\n"
             "  flow_control_ff=<num> - instructions before sync in fast-forward mode\n"
   ...
   template <typename T>
   bool parse(T& result, std::string key, const char* input)
   {
      const char* value;
      ...
      auto error = parseValue(result, value);
      if (error)
      {
         std::cerr << error;
         parsing_error = true;
   ...
   const char* parseValue(bool& result, const char *value)
   {
      if (!strcmp(value, "on") || !strcmp(value, "yes") ||
          !strcmp(value, "true") || !strcmp(value, "y")) {
         result = true;
         return nullptr;
      }

      if (!strcmp(value, "off") || !strcmp(value, "no") ||
          !strcmp(value, "false") || !strcmp(value, "n")) {
         result = false;
         return nullptr;
      }
   ...
   const char* parseValue(std::string& result, const char* value)
   {
      result = value;
      return nullptr;
   }
   ...
   template <typename T>
   const char* parseValue(T& result, const char* value)
   {
